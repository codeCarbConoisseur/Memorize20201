...
struct EmojiMemoryThemeEditor: View {
    ...
    @State private var cardBackColor: UIColor = .brown
    ...
    var body: some View {
        NavigationView {
                ...
                .onAppear() {
                    ...
                    self.backgroundColor = UIColor(self.theme.backgroundColor)
                    self.cardBackColor = UIColor(self.theme.cardBackColor)
                }
    ...
    static let colorPatches: [UIColor] = [
        ...
        .systemIndigo,  .systemPurple, .white
    ]
    ...
    var backgroundColorSection: some View {
        Section(header: Text("Background color").font(.headline)) {
            Grid(Self.colorPatches, id: \.self, viewForItem: backgroundView)
                .frame(height: self.colorGridHeight)
        }
    }
    ...
    private func colorItemView(_ color: UIColor) -> some View {
        ...
        return RoundedRectangle(cornerRadius: 4)
            ...
            .padding(2)
            .overlay(Group {
                if color.rgb == self.cardBackColor.rgb {
    ...
    private func backgroundView(_ color: UIColor) -> some View {
        var brightness: CGFloat = .zero
        color.getWhite(&brightness, alpha: nil)
        ...
        return RoundedRectangle(cornerRadius: 4)
            ...
            .padding(2)
            .overlay(Group {
                if color.rgb == self.backgroundColor.rgb {
                    Image(systemName: "checkmark.circle.fill")
                        .foregroundColor(brightness < 0.55 ? .white : .black)
                }
            })
    ...
    private func saveTheme() {
        ...
        store.updateTheme(for: theme, name: name, emoji: emojis, cardBackColor: cardBackColor.rgb, backgroundColor: backgroundColor.rgb, numberOfPairs: numberOfPairs)
